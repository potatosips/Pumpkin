...
mod tests {
    ...
    #[tokio::test]
    async fn loom_close_drop() {
        let player = TestPlayer::new();
        let mut handler = LoomScreenHandler::new(1, &player.inventory);

        // Put banner, dye, pattern in input slots
        let banner = ItemStack::new(1, &Item::WHITE_BANNER);
        let dye = ItemStack::new(1, &Item::RED_DYE);
        handler.input_inventory.set_stack(0, banner.clone()).await;
        handler.input_inventory.set_stack(1, dye.clone()).await;

        // Close loom - inputs should be dropped/given to player

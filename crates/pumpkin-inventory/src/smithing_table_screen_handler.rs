...
mod tests {
    ...
    #[tokio::test]
    async fn smithing_table_close_drop() {
        let player = TestPlayer::new();
        let mut handler = SmithingTableScreenHandler::new(1, &player.inventory);

        // Put template, base item, addition in input slots
        let template = ItemStack::new(1, &Item::NETHERITE_UPGRADE_SMITHING_TEMPLATE);
        let diamond_sword = ItemStack::new(1, &Item::DIAMOND_SWORD);
        let ingot = ItemStack::new(1, &Item::NETHERITE_INGOT);
        handler.input_inventory.set_stack(0, template.clone()).await;
        handler
            .input_inventory
            .set_stack(1, diamond_sword.clone())
            .await;
        handler.input_inventory.set_stack(2, ingot.clone()).await;

        // Close smithing table - inputs should be dropped/given to player
        ...
        assert!(handler.input_inventory.get_stack(2).await.is_empty());
